...
class _MainNavScreenState extends State<MainNavScreen> {
  ...
  void _onNotificationTapped(String? payload) {
    if (payload == 'action_dua_suhoor' || payload == 'action_dua_iftar') {
      ...
      // Если мы не на главном экране, переключаемся на него
      if (_selectedIndex != 0) {
        setState(() => _selectedIndex = 0);
      }
      // Показываем Дуа через небольшую паузу
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> screens = [
      ...
      QiblaScreen(
        ...
        // ИСПРАВЛЕНИЕ: Кнопка "Назад" на экране Киблы просто вернет нас на главную
        onMoveBack: () => setState(() => _selectedIndex = 0),
      ),
