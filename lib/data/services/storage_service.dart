...
class StorageService {
  ...
  // --- НОВЫЕ КЛЮЧИ ДЛЯ РУЧНОЙ ЛОКАЦИИ ---
  ...
  Future<void> saveTasbihCount(int count) async => await _prefs.setInt(_keyTasbihCount, count);

  int getTasbihIndex() => _prefs.getInt(_keyTasbihIndex) ?? 0;
  Future<void> saveTasbihIndex(int index) async => await _prefs.setInt(_keyTasbihIndex, index);
  ...
  Future<void> saveCity(String city) async => await _prefs.setString(_keyCity, city);

  String? getCountryCode() => _prefs.getString(_keyCountryCode);
  Future<void> saveCountryCode(String code) async => await _prefs.setString(_keyCountryCode, code);

  bool getIsManualLocation() => _prefs.getBool(_keyIsManualLocation) ?? false;
  Future<void> saveIsManualLocation(bool value) async => await _prefs.setBool(_keyIsManualLocation, value);

  double? getManualLat() => _prefs.getDouble(_keyManualLat);
  double? getManualLng() => _prefs.getDouble(_keyManualLng);

  // Комплексный метод для сохранения всей ручной локации за один раз
  Future<void> saveManualLocation(double lat, double lng, String city, String countryCode) async {
  ...
  // --- МЕТОДЫ ДЛЯ НАСТРОЕК (Язык и Молитвы) ---
  String? getLanguage() => _prefs.getString(_keyLanguage);
  Future<void> saveLanguage(String langCode) async => await _prefs.setString(_keyLanguage, langCode);

  bool getNotificationsEnabled() => _prefs.getBool(_keyNotifications) ?? true;
  Future<void> saveNotificationsEnabled(bool value) async => await _prefs.setBool(_keyNotifications, value);

  int getSuhoorOffset() => _prefs.getInt(_keySuhoorOffset) ?? 30;
  Future<void> saveSuhoorOffset(int minutes) async => await _prefs.setInt(_keySuhoorOffset, minutes);

  int getIftarOffset() => _prefs.getInt(_keyIftarOffset) ?? 0;
  Future<void> saveIftarOffset(int minutes) async => await _prefs.setInt(_keyIftarOffset, minutes);

  int? getMadhabIndex() => _prefs.getInt(_keyMadhab);
  Future<void> saveMadhabIndex(int index) async => await _prefs.setInt(_keyMadhab, index);

  int? getCalculationMethodIndex() => _prefs.getInt(_keyCalcMethod);
  Future<void> saveCalculationMethodIndex(int index) async => await _prefs.setInt(_keyCalcMethod, index);
}